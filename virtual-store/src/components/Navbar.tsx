import NavButton from "./NavButton";
import { Link } from "react-router-dom";

interface NavbarProp {
  search: boolean;
}

function Navbar({search}: NavbarProp) {
  if (search) {
  return (
    <>
      <header className="w-full h-[150px] bg-[#ff3b3c] p-2 flex flex-col items-center "   > {/* header */}
        <div className="w-[1080px] flex justify-between items-center flex-grow "> {/* header-container */}
          <Link to="/" className="w-1/3 flex items-center flex-grow-1 "> {/* header-logo */}
            <img
              className="h-[50px]" /* header-logo-img */
              src="/tiendamia-logo.svg"
              width="218"
              alt="Logo store"
            />
          </Link>
          <form className="w-1/3 flex items-center flex-grow-1"> {/* header-form */}
            <input
              className="h-[60px] rounded-2xl border-none w-full p-10px m-0 mx-5 text-sm text-center " /* header-input */
              type="text"
              placeholder="Search"
              id="search"
            />
          </form>
          <ul className="w-1/3 flex items-center grow justify-end list-none" > { /*header-social */ }
            <li id="facebook" className="w-[50px] h-[50px]  "> {/* header-li */}
              <a className="w-[50px] h-[50px] " /* header-a */ href="https://facebook.com">
                <img
                  className="h-[50px] w-[50px] "   /* header-social-img */
                  src="data:image/png;base64,"
                />
              </a>
            </li>
            <li id="instagram" className="w-[50px] h-[50px]" /* header-li */>
              <a className="w-[50px] h-[50px] " /* header-a  */href="https://instagram.com">
                <img
                  className="w-[50px] h-[50px] " /* header-social-img */
                  src="data:image/png;base64,"
                />
              </a>
            </li>
            <li id="cart" className="w-[50px] h-[50px] "  /* header-li */ >
              <Link className="w-[50px] h-[50px] " /* header-a */ to="./Cart">
                <img
                  className="w-[50px] h-[50px] " /* header-social-img */
                  src="data:image/png;base64,"
                />
              </Link>
            </li>
          </ul>
        </div>
        <nav className="w-[1080px] p-4 px4 bg-[#ff3b3c] flex justify-between list-none text-sm  " /* NAV */ id="navbar">
          <NavButton title="Ofertas" link="#" />
          <NavButton title="Cómo comprar" link="#" />
          <NavButton title="Costos y tarifas" link="#" />
          <NavButton title="Mis pedidos" link="#" />
          <NavButton title="Garantía" link="#" />
        </nav>
      </header>
    </>
  );
} 
return (
  <>
  <header className="" /* HEADER */  >
        <div className="w-[1080px] flex justify-between items-center flex-grow" > {/* header-container */}
          <Link to="/" className="w-1/3 flex items-center flex-grow-1" > {/* header-logo */}
            <img
              className="h-[50px]" /* header-logo-img */
              src="/tiendamia-logo.svg"
              width="218"
              alt="Logo store"
            />
          </Link>
          <form className="w-1/3 flex items-center flex-grow-1" > {/* header-form */}
            <input
              className="h-[60px] rounded-2xl border-none w-full p-10px m-0 mx-5 text-sm text-center" /* header-imput */
              type="text"
              placeholder="Search"
              id="search"
            />
          </form>
          <ul className="w-1/3 flex items-center grow justify-end list-none "> {/* header-social */}
            <li id="facebook" className="w-[50px] h-[50px]"> {/* header-li */}
              <a className="w-[50px] h-[50px]" /* header-a */ href="https://facebook.com">
                <img
                  className="w-[50px] h-[50px]" /* header-social-img */
                  src="data:image/png;base64,"
                />
              </a>
            </li>
            <li id="instagram" className="w-[50px] h-[50px]" > {/* header-li */}
              <a className="w-[50px] h-[50px]"  /* header-a */ href="https://instagram.com">
                <img
                  className="w-[50px] h-[50px]" /* header-social-img */
                  src="data:image/png;base64,"
                />
              </a>
            </li>
            <li id="cart" className="w-[50px] h-[50px]"> {/* header-li */}
              <Link className="w-[50px] h-[50px]" /* header-a */ to="./Cart">
                <img
                  className="w-[50px] h-[50px]" /* header-social-img */
                  src="data:image/png;base64,"
                />
              </Link>
            </li>
          </ul>
        </div>
        <nav className="w-[1080px] p-4 px4 bg-[#ff3b3c] flex justify-between list-none text-sm" /* NAVBAR */ id="navbar">
          <NavButton title="Ofertas" link="#" />
          <NavButton title="Cómo comprar" link="#" />
          <NavButton title="Costos y tarifas" link="#" />
          <NavButton title="Mis pedidos" link="#" />
          <NavButton title="Garantía" link="#" />
        </nav>
      </header>
  
  </>
)
}
export default Navbar;
