

import  { useState } from 'react';
import NavButton from "./NavButton";
import { Link } from "react-router-dom";

interface NavbarProp {
  search: boolean;
}

function Navbar({ search }: NavbarProp) {
  const [menuOpen, setMenuOpen] = useState(false);

  const toggleMenu = () => {
    setMenuOpen(!menuOpen);
  };

  return (
    <>
      <header className="w-full h-[150px] bg-[#ff3b3c] p-2 flex flex-col items-center"> {/* header */}
        <div className="w-full max-w-[1080px] flex justify-between items-center flex-grow px-4 "> {/* header-container */}
          <Link to="/" className=" w-1/3 flex items-center flex-grow-1"> {/* header-logo */}
            <img
              className="h-[50px]" /* header-logo-img */
              src="/tiendamia-logo.svg"
              width="218"
              alt="Logo store"
            />
          </Link>
          {search && (
            <form className="hidden md:flex w-1/3 items-center flex-grow-1"> {/* header-form */}
              <input
                className="h-[60px] rounded-2xl border-none w-full p-2 text-sm text-center" /* header-input */
                type="text"
                placeholder="Buscar..."
                id="search"
              />
            </form>
          )}
          <button
            className="md:hidden text-white"
            onClick={toggleMenu}
          >
            ☰
          </button>
          <ul className="hidden md:flex w-1/3 items-center grow justify-end list-none">
            <li id="facebook" className="w-[50px] h-[50px]"> {/* header-li */}
              <a className="w-[50px] h-[50px]" /* header-a */ href="https://facebook.com">
                <img
                  className="w-[50px] h-[50px]" /* header-social-img */
                  src="data:image/png;base64,"
                />
              </a>
            </li>
            <li id="instagram" className="w-[50px] h-[50px]" /* header-li */>
              <a className="w-[50px] h-[50px]" /* header-a*/ href="https://instagram.com">
                <img
                  className="w-[50px] h-[50px]" /* header-social-img */
                  src="data:image/png;base64,"
                />
              </a>
            </li>
            <li id="cart" className="w-[50px] h-[50px]" /* header-li */>
              <Link className="w-[50px] h-[50px]" /* header-a */ to="../Cart">
                <img
                  className="w-[50px] h-[50px]" /* header-social-img*/
                  src="data:image/png;base64,"
                />
              </Link>
            </li>
          </ul>
        </div>
        {menuOpen && (
          <nav className=
          
          {` md:static md:hidden w-full top-0 left-0 flex flex-col items-center p-4 md:w-auto bg-[#ff3b3b80] md:bg-transparent transform ${ menuOpen ? 'translate-y-0' : '-translate-y-full'
          } md:translate-y-0 transition-transform duration-1000 ease-in-out`}
          >
            <NavButton title="Ofertas" link="#" />
            <NavButton title="Cómo comprar" link="#" />
            <NavButton title="Costos y tarifas" link="#" />
            <NavButton title="Mis pedidos" link="#" />
            <NavButton title="Garantía" link="#" />
          </nav>
        )}
        <nav className="hidden md:flex w-full max-w-[1080px] p-4 px-4 bg-[#ff3b3c] justify-between list-none text-sm" id="navbar">
          <NavButton title="Ofertas" link="#" />
          <NavButton title="Cómo comprar" link="#" />
          <NavButton title="Costos y tarifas" link="#" />
          <NavButton title="Mis pedidos" link="#" />
          <NavButton title="Garantía" link="#" />
        </nav>
      </header>
    </>
  );
}

export default Navbar;


